...
function TopicStudyInner() {
  ...
  const topicDecoded = topicRaw ? decodeURIComponent(topicRaw) : ''
  ...
  const tutorContext = topicDecoded
    ? `You are a Nigerian exam tutor helping a student study "${topicDecoded}" in ${subjectLabel} for ${examLabel}. All your explanations, examples and questions should be based on the Nigerian ${examLabel} syllabus. Be clear, concise and exam-focused.`
    : ''
  ...
  useEffect(() => {
    if (!topicDecoded || !exam || !subjectKey || !tutorContext) return
    if (initialTutorSent.current) return
    initialTutorSent.current = true
    const first = `Explain ${topicDecoded} to me in simple terms with examples relevant to the ${examLabel} exam`
    void (async () => {
      setTutorSending(true)
      const userMessage: ChatMessage = {
        id: `${Date.now()}-u0`,
  ...
  }, [topicDecoded, exam, subjectKey, tutorContext, examLabel])
  ...
  const bumpProgress = () => {
    const k = progressStorageKey(exam, subjectKey, topicDecoded)
  ...
  const invalid = !validExam || !validSubject || !topicDecoded
  ...
  const practicePanel = (
    <div className={`${CARD} flex flex-col min-h-[320px] lg:min-h-[calc(100vh-220px)]`}>
      <div className="p-3 border-b border-[#E8EAED] dark:border-gray-700 flex flex-wrap items-center gap-2 justify-between">
        <div>
          <p className="font-semibold text-gray-900 dark:text-white">Practice: {topicDecoded}</p>
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          {invalid ? (
            <div className={`${CARD} p-8 text-center`}>
              <p className="text-gray-600 dark:text-gray-400 mb-4">Missing topic parameters.</p>
...
function TopicFallback() {
  return (
    <div className="min-h-[40vh] flex items-center justify-center bg-[#F7F8FA] dark:bg-slate-950">
      <FiLoader className="animate-spin text-2xl text-violet-500" />
    </div>
  )
}
