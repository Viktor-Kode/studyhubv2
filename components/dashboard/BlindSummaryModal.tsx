...
export default function BlindSummaryModal({
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 bg-slate-900/80 backdrop-blur-md overflow-y-auto animate-fadeIn">
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 border border-purple-100 dark:border-purple-900/40 rounded-3xl shadow-2xl overflow-hidden my-8">
          ...
          <div className="flex items-center gap-3 z-10">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 text-[10px] font-extrabold uppercase tracking-wider bg-yellow-400 text-purple-950 rounded-full shadow-sm">
                  Active Recall
                </span>
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          {!submitted ? (
            <>
              {/* Context Banner */}
              <div className="p-4 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800/50 rounded-2xl flex items-start gap-3">
                <div className="p-2 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-xl mt-0.5">
                  <FiLock className="w-5 h-5" />
                </div>
                <div className="text-sm">
                  <p className="font-semibold text-slate-800 dark:text-slate-200">
                    Content hidden for recall testing: <span className="text-purple-600 dark:text-purple-400">{title}</span>
                  </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-extrabold uppercase tracking-wide text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                    <BiBrain className="w-4 h-4 text-purple-500" />
                    Type 3 Main Points Below:
                  </label>
                  <span className="text-xs font-semibold text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-900/50 px-2.5 py-0.5 rounded-full">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full px-3.5 py-2.5 text-sm bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all dark:text-slate-100 placeholder:text-slate-400"
                  />
                </div>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all transform active:scale-95"
                  >
                    {isAwarding ? (
                      <>
            ...
            <div className="space-y-6 animate-fadeIn">
              {/* Celebration Card */}
              <div className="p-6 bg-gradient-to-br from-purple-500/10 via-indigo-500/10 to-emerald-500/10 border border-purple-200 dark:border-purple-800/60 rounded-3xl text-center space-y-3">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2.5">
                  {[point1, point2, point3].filter(Boolean).map((pt, idx) => (
                    ...
                    >
                      <span className="w-5 h-5 rounded-full bg-purple-600/20 text-purple-600 dark:text-purple-400 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
                ...
                {showOriginal && originalSummaryText && (
                  <div className="mt-4 p-4 bg-purple-50/70 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800/40 rounded-2xl space-y-2 max-h-60 overflow-y-auto">
                    ...
                    <div className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                      <ReactMarkdown>{originalSummaryText}</ReactMarkdown>
                    </div>
                  </div>
                )}
              </div>
